...
<template>
    <transition name="show-unlock">
        <div class="unlock-body-con" v-if="showUnlock" @keydown.enter="handleUnlock">
            ...
            <div class="unlock-locking-tip-con">欢迎您！<span class="ul-text">{{userName}}</span></div>
        </div>
    </transition>
</template>
...
export default {
    ...
    computed: {
        avatorPath () {
          return this.$store.state.user.member.avatar||'../static/avatar.jpg';
        },
        userName () {
          return this.$store.state.user.member.nick_name;
        }
    },
    methods: {
        handleClickAvator () {
        ...
        handleUnlock () {
            if (localStorage.getItem('mzmy_pwd') === MD5(this.password)) {
                this.avatorLeft = '0px';
