  ...
  <div>
            ...
            <table class="table table-bordered table-condensed kk-table">
                <thead>
                    <tr class="info">
                    <th>会员</th>
                    <th width="120px">会员卡编号</th>
                    <th width="100px">卡项</th>
                    <th>折扣</th>
                    <th>卡金余额</th>
                    <th width="100px">充值额度</th>
                    <th width="100px">赠送金</th>                        
                    <th>
                        销售员工
                    </th>
                    </tr>
                ...
                <tbody>
                    <tr v-if="member.id&&member.card.id">
                        ...
                        <td>套餐：{{memberCard.getGroupServiceDiscountRateWithLabel()}},
                            服务项目：{{memberCard.getItemDiscountRateWithLabel()}}
                        </td>
                        <td>{{member.card.amount}}</td>
...
import MemberCard from "../../../model/StoreMember/MemberCard";
let _this;	
export default {
    name: 'cashier-hhcz',
    data () {
    ...
    computed: {
        memberCard() {
            ...
            if(this.member.card && this.member.card.id) {
                if(this.currentMemberCard.id && this.currentMemberCard.id == this.member.card.id) {
                    return this.currentMemberCard;
                }
                this.currentMemberCard = new MemberCard();
                this.currentMemberCard.parseJSONObject(this.member.card);
            } else {
                this.currentMemberCard = new MemberCard();
            }
            return this.currentMemberCard;
        ...
        minGivenAmount () {
            let n = 0;
            if(this.member.card&&this.member.card.product_id){
                let data = this.cardList.filter(e=>{
                    return e.id === this.member.card.product_id;
                })
                if(data.length){
        ...
        async dataInit (){
            ...
            if(this.order.id){
                ...
                if(this.member.id){
                    ...
                    this.formItem.items[0].sku = this.member.number;
                    if(this.member.card.id){
                        this.formItem.items[0].product_id = this.member.card.product_id; 
                        let data = this.cardList.filter(e=>{
                            return e.id === this.member.card.product_id;
                        })
                        if(data.length){
